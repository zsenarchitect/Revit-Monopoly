...
if True:
    ...
    if getattr(cate_graphic, graphic_item) != getattr(element_graphic, graphic_item)
    
    .AsInteger()

    list_to_system_list(list, type = "ElementId", use_IList = False)

    wall_ids = [x.Id for x in my_walls]
    selection_ids = uidoc.Selection.SetElementIds (EA_UTILITY.list_to_system_list(wall_ids))
    ...
    """
    """
    doc.GetDefaultElementTypeId(DB.ElementTypeGroup.TextNoteType), can get deafault element type from element type group enumeration
    """
    """
    import imp
    ref_module = imp.load_source("make walltype filter_script", r'L:\4b_Applied Computing\01_Revit\04_Tools\08_EA Extensions\Published\ENNEAD.extension\Ennead.tab\Tailor Shop.panel\misc1.stack\Proj 2135 Part2.pulldown\52_make walltype_filter and assign.pushbutton\make walltype filter_script.py')

    print ref_module.desired_filters
    """
    ...
    _HostApplication.get_postable_commands() #Return list of postable commands.

    _HostApplication.post_command(command_id) #Request Revit to run a command
    Parameters:    command_id (str) – command identifier e.g. ID_REVIT_SAVE_AS_TEMPLATE
    """

    """
    res, check =  EA_UTILITY.dialogue( title = __doc__,
                                        main_text = "main_text",
                                        sub_text = "sub_text",
                                        use_progress_bar = True,
                                        options = [["opt 1","description long long long long"], "opt 2"],
                                        extra_check_box_text = "qq")
    print res, check
